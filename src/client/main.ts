...
const userMessages$ = submitEvents$.pipe(
    ...
    map((e: Event) => {const messageInput: HTMLInputElement = ((e.currentTarget as HTMLFormElement).querySelector('input[name="message"]')!);
...
userMessages$.subscribe(message => sendMessage(message));
